{{--
 * [POR CONFIRMAR] - resources/views/webmaster/actividades/editar.blade.php
--}}
@extends('layouts.app')
@section('header')
...
<div class="min-h-screen bg-gray-50" style="padding-top: 3rem;">
    ...
    <div class="mx-auto flex w-full max-w-full gap-6 px-1 py-6 md:px-2">
        ...
        <div class="flex-1" style="padding-top: 1rem;">
            <div class="mb-6">
                <h1 class="text-2xl font-semibold text-gray-900">Editar Actividad</h1>
                <p class="mt-2 text-sm text-gray-600">Modifica los datos de la actividad</p>
            </div>

            <div class="max-w-2xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                <form method="POST" action="{{ route('webmaster.actividades.actualizar', $actividad->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Nombre de la Actividad</label>
                        <input type="text" name="nombre" value="{{ old('nombre', $actividad->nombre) }}" required class="mt-2 w-full rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                        @error('nombre')
                    ...
                    <div>
                        ...
                        <textarea name="descripcion" rows="4" class="mt-2 w-full rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('descripcion', $actividad->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="imagen_url" value="{{ old('imagen_url', $actividad->imagen_url) }}" placeholder="https://ejemplo.com/imagen.jpg" class="mt-2 w-full rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                        @error('imagen_url')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
